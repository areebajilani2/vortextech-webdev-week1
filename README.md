# Personal Portfolio Page — Week 1

A single-page personal portfolio site built with plain HTML5 and CSS3, as part of the
VortexTech Web Development Internship, Week 1 (Beginner).

## What's included
- `index.html` — page structure: Header, About, Skills, Contact, Footer 
- `style.css` — all styling , including a Flexbox layout for the skills
  list, a consistent 3-color scheme, and a media query for mobile responsiveness

## How to run it
1. Download or clone this repository.
2. Open `index.html` directly in any web browser (double-click the file, or right-click →
   Open With → your browser).
3. No build steps, servers, or dependencies required.


